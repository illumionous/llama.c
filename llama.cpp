...
// helpers
//
extern void save_tensor_data(struct gguf_context* ctx, uint32_t index, const char* directory);
extern void save_tensors_meta_info(struct gguf_context* ctx, const char* directory);

extern uint32_t get_gguf_tensor_count(struct gguf_context* ctx);
extern struct gguf_tensor_info* get_gguf_tensor_info(struct gguf_context* ctx, uint32_t index);
void write_tensor_to_file(const void * data, size_t size, const std::string & file_path) {
    ...
    if (!out_file.good()) {
        throw std::runtime_error("写入张量数据到文件失败：" + file_path);
    }
}

// 保存所有张量数据到指定目录的函数
void save_all_tensors(const struct gguf_context * ctx, const std::string & directory) {
    const uint8_t * base_data = static_cast<const uint8_t *>(gguf_get_data(ctx));
    ...
    for (int i = 0; i < n_tensors; ++i) {
        ...
        // 计算张量数据的起始指针
        const uint8_t * tensor_data = base_data + data_offset + tensor_offset;

        // 构建文件路径
        std::string file_path = directory + "/" + std::string(tensor_name) + ".bin";  // 为文件添加.bin扩展名

        // 将张量数据写入文件
        write_tensor_to_file(tensor_data, tensor_size, file_path);
    }
}

// Convert the entire tensor in one go on the main thread
static void convert_and_save_tensor(
    ...
    if (tensor->type == GGML_TYPE_F32) {
        // 张量已经是F32类型，直接保存
        std::ofstream outfile(output_folder + "/" + name + ".bin", std::ios::binary);
        if (!outfile.is_open()) {
            throw std::runtime_error("Unable to open file for writing: " + output_folder + "/" + name + ".bin");
        }
        outfile.write(reinterpret_cast<const char*>(tensor->data), nelements * sizeof(float));
        outfile.close();
    } else if (tensor->type == GGML_TYPE_F16 || ggml_is_quantized(tensor->type)) {
        // 创建一个缓冲区来存储转换后的数据
        std::vector<float> f32_output(nelements);

        // 检查张量类型并执行转换
        if (tensor->type == GGML_TYPE_F16) {
            // 调用库函数转换半精度浮点数到单精度浮点数
            ggml_fp16_to_fp32_row((ggml_fp16_t *)tensor->data, f32_output.data(), nelements);
        } else {
            // 获取类型特征
            ggml_type_traits_t qtype = ggml_internal_get_type_traits(tensor->type);
            // 转换量化数据到浮点数
            qtype.to_float(tensor->data, f32_output.data(), nelements);
        }

        // 构建保存文件的完整路径
        std::string file_path = output_folder + "/" + name + ".bin";
        ...
        outfile.write(reinterpret_cast<const char*>(f32_output.data()), nelements * sizeof(float));
        outfile.close();
    } else {
        throw std::runtime_error("Unsupported tensor type for conversion");
    }
}
...
struct llama_model_loader {
    ...
    void save_data_for(const char * name, struct ggml_context * ctx) const {
        // 构建文件的完整路径
        std::string directory = "/home/patrick/test/load_tensor_q3k/";
        std::string file_path = directory + std::string(name) + ".bin";

        // 查找张量
        struct ggml_tensor * tensor = ggml_get_tensor(ctx, name);
        if (!tensor) {
            throw std::runtime_error(format("%s: tensor '%s' not found in the context", __func__, name));
        }
        ...
        std::ofstream out_file(file_path, std::ios::binary);
        if (!out_file) {
            throw std::runtime_error(format("%s: failed to open file '%s'", __func__, file_path.c_str()));
        }

        // 写入张量数据到文件
        out_file.write(reinterpret_cast<const char*>(tensor->data), ggml_nbytes(tensor));
        out_file.close();

        if (!out_file.good()) {
            throw std::runtime_error(format("%s: failed to write data for tensor '%s'", __func__, name));
        }
    }
    ...
    void load_all_data(struct ggml_context * ctx, llama_progress_callback progress_callback, void * progress_callback_user_data, llama_mlock * lmlock) {
        ...
        for (int i = 0; i < gguf_get_n_tensors(ctx_gguf); i++) {
            ...
            load_data_for(cur);
            convert_and_save_tensor(cur,ggml_get_name(cur),"/home/patrick/test/dequantizetensor");
            // save_data_for(ggml_get_name(cur), ctx);
            switch (cur->backend) {
...
#endif
    std::cout<<"fine are you ok"<<std::endl;
...
void save_tensor_info(const std::string& filename, const std::string& tensor_name, int64_t rows, int64_t cols) {
    std::ofstream file(filename, std::ios::app);  // 以追加模式打开文件
    if (file.is_open()) {
        file << "Tensor Name: " << tensor_name << ", Rows: " << rows << ", Cols: " << cols << '\n';
        file.close();
    } else {
        std::cerr << "Failed to open file " << filename << '\n';
    }
}
void save_token_embeddings(const ggml_tensor* tensor) {
    // 检查张量和数据是否有效
    if (!tensor || !tensor->data) {
        std::cerr << "Invalid tensor or data is null\n";
        return;
    }
    std::cout<<"tensortypeis"<<tensor->type<<std::endl;
    std::cout<<"tensor "<<tensor->n_dims<<std::endl;
    // 确定保存路径和文件名
    const std::string save_path = "/home/patrick/test/temp";
    std::string filename = save_path + "/token_embeddings.bin";

    // 获取数据大小
    int64_t data_size = ggml_nbytes(tensor);
    
    // 获取反量化函数
    ggml_to_float_t const dequantize_row_q = ggml_internal_get_type_traits(tensor->type).to_float;
    
    // 计算反量化后的数据大小
    int64_t dequantized_size = ggml_nelements(tensor) * sizeof(float);
    size_t typesize =ggml_internal_get_type_traits(tensor->type).type_size;
    // 分配内存以保存反量化后的数据
    float* dequantized_data = (float*)malloc(dequantized_size);
    if (!dequantized_data) {
        std::cerr << "Failed to allocate memory for dequantized data\n";
        return;
    }
    int nr=ggml_nrows(tensor);
    std::cout<<"ne[0]is"<<tensor->ne[0]<<std::endl;
    std::cout<<"ne[1]is"<<tensor->ne[1]<<std::endl;
    // 执行反量化过程
    for (int i = 0; i < nr; ++i) {
        // std::cout << "Processing row " << i << '\n';  // 输出正在处理的行号
        dequantize_row_q(
                (const void *) ((char *) tensor->data + i * tensor->nb[1]),
                (float *) ((char *) dequantized_data + i * 4*tensor->ne[0]),
                tensor->ne[0]);
    }
    ...
    if (file.is_open()) {
        // 写入反量化后的数据
        file.write((const char*)dequantized_data, dequantized_size);
        file.close();
...
void save_tensor_data(const ggml_tensor* tensor) {
    std::lock_guard<std::mutex> guard(file_mutex);
    std::cout << "Entering save_tensor_data\n";  // 添加在函数开始处
    ...
    const std::string save_path="/home/patrick/test/loadfileq3k";

    std::string filename = save_path + "/" + tensor->name + ".bin";
    ...
    size_t typesize =ggml_internal_get_type_traits(tensor->type).type_size;
    int64_t dequantized_size = ggml_nelements(tensor) * sizeof(float);
    std::cout << "Dequantized size: " << dequantized_size << '\n';  // 输出反量化后的数据大小
    ...
    if (data_size == dequantized_size) {
        // data_size 与 dequantized_size 相等，直接将原始数据写入文件
        std::ofstream file(filename, std::ios::binary);
        std::string tensor_name = tensor->name;
        int64_t rows = tensor->ne[0];
        int64_t cols = tensor->ne[1];
        if (file.is_open()) {
            std::cout << "File is open, writing data\n";  // 输出文件打开成功，正在写入数据
            file.write((const char*)tensor->data, data_size);
            file.close();
            std::cout << "Saved " << filename << '\n';
        } else {
            std::cerr << "Failed to open file " << filename << '\n';
        ...
        // 在保存原始数据后，保存张量信息
        std::string info_filename = save_path + "/tensor_info.txt";
        save_tensor_info(info_filename, tensor_name, rows, cols);

        std::cout << "Exiting save_tensor_data\n";  // 添加在函数结束处
        return;  // 退出函数
    }
    std::cout<<"nr is"<<std::endl;
    for (int i = 0; i < nr; ++i) {
        std::cout << "Processing row " << i << '\n';  // 输出正在处理的行号
        dequantize_row_q(
                (const void *) ((char *) tensor->data + i * tensor->nb[1]),
...
void print_and_save_model_data(const llama_model& model) {
    // 遍历并保存所有 layers 中的数据
    for (size_t i = 0; i < model.layers.size(); ++i) {
        const llama_layer& layer = model.layers[i];
        std::cout << "Saving data for Layer " << i << ":\n";
        
        // 保存该层的所有张量数据
        save_tensor_data(layer.attn_norm);
        save_tensor_data(layer.wq);
        save_tensor_data(layer.wk);
        save_tensor_data(layer.wv);
        save_tensor_data(layer.wo);
        save_tensor_data(layer.wqkv);
        save_tensor_data(layer.ffn_norm);
        save_tensor_data(layer.ffn_gate);
        save_tensor_data(layer.ffn_down);
        save_tensor_data(layer.ffn_up);
    }

    // 保存其他 ggml_tensor 结构中的数据
    save_tensor_data(model.tok_embd);
    save_tensor_data(model.output_norm);
    save_tensor_data(model.output_norm_b);
    save_tensor_data(model.output);
}
struct llm_build_context {
    ...
    struct ggml_cgraph * build_llama() {
        ...
        // print_and_save_model_data(model);
        struct ggml_tensor * cur;
        struct ggml_tensor * inpL;
        // print_and_save_model_data(model);
...
static void llama_convert_tensor_(
    struct ggml_tensor * tensor, std::vector<no_init<float>> & output, const size_t nelements
) {
    if (output.size() < nelements) {
        output.resize(nelements);
    }
